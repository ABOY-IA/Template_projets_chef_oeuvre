...
async def get_admin_user(
    x_user: str = Header(...), db: AsyncSession = Depends(get_db)
) -> User:
...
async def list_all_users(
    ...
    logger.info(
        f"Admin {admin.username} a listé tous les utilisateurs ({len(users)} trouvés)"
    )
...
async def delete_user(
    ...
    if not user:
        logger.warning(
            f"Tentative de suppression d'utilisateur inexistant (id={user_id}) par admin {admin.username}"
        )
        raise HTTPException(
    ...
    await db.commit()
    logger.info(
        f"Admin {admin.username} a supprimé l'utilisateur id={user_id}"
    )
    return
